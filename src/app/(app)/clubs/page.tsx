...
function ClubCard({ club, isMember, isAdmin }: { club: Club, isMember: boolean, isAdmin: boolean }) {
                    ...
                    <CardDescription className="flex-grow">{club.description}</CardDescription>
                     <div className="flex justify-between items-center mt-4 text-sm text-muted-foreground border-t pt-4">
                        <div className="flex items-center gap-2">
                         ...
                         {isMember && (
                            <span className="text-xs font-semibold text-primary">You're a member</span>
                         )}
                    </div>
                </CardContent>
            </Card>
...
export default function ExploreClubsPage() {
    ...
    const myClubIds = useMemoFirebase(() => memberships?.map(m => m.clubId) || [], [memberships]);
        ...
        return (
            <div className="container mx-auto">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h1 className="text-4xl font-bold tracking-tight">Explore Clubs</h1>
                        <p className="text-muted-foreground mt-1">Discover student organizations on campus.</p>
                    </div>
                </div>
                    ...
                    {[...Array(6)].map((_, i) => (
                        <Card key={i} className="h-full flex flex-col">
                             ...
                             <CardContent className="space-y-2">
                                ...
                                <div className="w-2/3 h-4 rounded bg-muted animate-pulse"></div>
                             </CardContent>
                        </Card>
                    ))}
                </div>
